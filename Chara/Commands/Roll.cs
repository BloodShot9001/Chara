...
namespace Chara.Commands
{
    public class Roll : ModuleBase<SocketCommandContext>
    {
        ...
        {
            await Context.Channel.SendMessageAsync("Yes you typed /roll.");
        }
        [Command("roll")]
        public async Task roll(params String[] original)
        {
            await Context.Channel.SendMessageAsync("Started roll");
            String tempString = String.Join("", original);
            await Context.Channel.SendMessageAsync("Joined tempString");
            s = Regex.Split(tempString, @"([+-])");
            await Context.Channel.SendMessageAsync("Split again");
